...
def fitness_function(schedule, ratings_data, schedule_length):
    ...
    Scaled ×10 for report-friendly numbers.
    """
    total_rating = 0
    for time_slot, program in enumerate(schedule):
        if program in ratings_data:
            if time_slot < len(ratings_data[program]['hourly']):
                # Scale hourly ratings ×10
                total_rating += ratings_data[program]['hourly'][time_slot] * 10
    return total_rating
...
if ratings:
    ...
    if st.sidebar.button("🚀 Run All 3 Trials"):
        ...
        hourly_ratings_1 = [ratings[program]['hourly'][i]*10 for i, program in enumerate(schedule_1)]
        final_ratings_1 = [ratings[program]['final'] for program in schedule_1]
        df_1 = pd.DataFrame({
        ...
        st.dataframe(df_1)
        st.write(f"*Best Fitness Score (Scaled Total Hourly Rating): {fitness_1:.2f}*")
        st.markdown("---")
        ...
        hourly_ratings_2 = [ratings[program]['hourly'][i]*10 for i, program in enumerate(schedule_2)]
        final_ratings_2 = [ratings[program]['final'] for program in schedule_2]
        df_2 = pd.DataFrame({
            "Time Slot": [f"{h:02d}:00" for h in all_time_slots],
        ...
        st.dataframe(df_2)
        st.write(f"*Best Fitness Score (Scaled Total Hourly Rating): {fitness_2:.2f}*")
        st.markdown("---")
        ...
        hourly_ratings_3 = [ratings[program]['hourly'][i]*10 for i, program in enumerate(schedule_3)]
        final_ratings_3 = [ratings[program]['final'] for program in schedule_3]
        df_3 = pd.DataFrame({
            "Time Slot": [f"{h:02d}:00" for h in all_time_slots],
        ...
        st.dataframe(df_3)
        st.write(f"*Best Fitness Score (Scaled Total Hourly Rating): {fitness_3:.2f}*")
        st.markdown("---")
